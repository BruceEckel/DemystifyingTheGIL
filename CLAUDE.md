...
- **`stats_race.py`** — A stats accumulator with `count` and `total` updated on separate lines. They can diverge under free-threading, producing a wrong or crashing `mean()`.
